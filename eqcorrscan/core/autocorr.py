...
def _segment_data(stream, length, lag, max_lag=200, cores=1):
    ...
    iterables = [range(n_segments) for _ in range(len(stations))]
    ...
    for t in itertools.product(*iterables):
        template = Stream()
        if any(abs((_t * lag) - (min(t) * lag)) > max_lag for _t in t):
            continue
        for lagmult, station in zip(t, stations):
...
def autocorr(stream, length, lag, threshold, threshold_type, plotvar,
             ...
             output_cat=False, extract_detections=False, arg_check=True):
    """
    ...
    templates = _segment_data(stream=stream, length=length, lag=lag,
                              cores=cores)
    template_names = ['autocorr_' + i for i in range(len(templates))]
    candidates = match_filter.match_filter(
        template_names=template_names, template_list=templates, st=stream,
        threshold=threshold, threshold_type=threshold_type, plotvar=plotvar,
        plotdir='.', cores=1, debug=0, plot_format='png',
        output_cat=False, extract_detections=False, arg_check=True)
    return candidates